...
// denyByNameAccess allows everything except names in deny, and denies them only
// through BatchCheck so it exercises the batched authorization path.
type denyByNameAccess struct{ deny map[string]struct{} }

func (denyByNameAccess) Check(context.Context, claims.AuthInfo, claims.CheckRequest, string) (claims.CheckResponse, error) {
...
func TestListWithSelectorsBatchAuthorization(t *testing.T) {
	ctx := identity.WithServiceIdentityContext(context.Background(), 1)
	searchClient := &stubSearchClient{resp: &resourcepb.ResourceSearchResponse{
		ResourceVersion: 100,
		Results: &resourcepb.ResourceTable{Rows: []*resourcepb.ResourceTableRow{
	...
	require.NoError(t, err)
	require.Len(t, resp.Items, 2, "the batch-denied row must be filtered out")
	// One BatchCheck for the chunk, not one Check per row.
	require.Equal(t, 1, backend.batchCalls)
}
