...
export function AppChrome({ children }: Props) {
  ...
  const styles = useStyles2(
    ...
    state.chromeless ? 0 : noticeHeight
  );
...
const getStyles = (
  ...
  const chromeOffset = headerLevels * headerHeight + noticeHeight;
  return {
    content: css({
      label: 'page-content',
      display: 'flex',
    ...
    scopesDashboardsContainer: css({
      position: 'fixed',
      height: `calc(100% - ${headerHeight + noticeHeight}px)`,
      zIndex: 1,
    }),
