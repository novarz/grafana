...
import { fieldReducers, ReducerID, reduceField, defaultCalcs, getFieldTypeForReducer } from './fieldReducer';
...
describe('fieldReducer.ts', () => {
  ...
  describe('getFieldTypeForReducer', () => {
    it.each`
      ...
      ${'allIsZero'}     | ${'boolean'}
      ${'p70'}           | ${'string'}
    `('Reducer $reducer should return the field type of $expectedType', ({ reducer, expectedType }) => {
      // no automatic conversion uses string, so if its string, its using the fallback
      const result = getFieldTypeForReducer(reducer, FieldType.string);
